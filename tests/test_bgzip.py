...
class TestBGZipReader(unittest.TestCase):
    ...
    def _test_inflater(self,
                       expected_data: bytes,
                       chunks: List[bytes],
                       buf_size: Optional[int]=None,
                       atomic: bool=False):
        remaining_chunks = chunks.copy()
        with bgzip.Inflater(buf_size, atomic=atomic) as inflater:
            remaining_chunks = remaining_chunks.copy()
            reinflated_data = b""
            while remaining_chunks:
                inflate_info = inflater.inflate(remaining_chunks)
    ...
    def test_inflater(self):
        expected_data = os.urandom((2 * bgzip.bgu.block_batch_size + 1) * bgzip.bgu.block_data_inflated_size)

        with bgzip.Deflater() as deflater:
        ...
        with self.subTest("all blocks"):
            self._test_inflater(expected_data, [bytes(b) for b in deflated_blocks])

        with self.subTest("chunked blocks"):
            self._test_inflater(expected_data, [b"".join(chunk)
                                for chunk in _randomly_chunked(deflated_blocks)])

        with self.subTest("initial large chunk"):
            self._test_inflater(expected_data, [b"".join(deflated_blocks[:-1]), deflated_blocks[-1]])

        with self.subTest("trailing large chunk"):
            self._test_inflater(expected_data, [deflated_blocks[0], b"".join(deflated_blocks[1:])])

        with self.subTest("small inflate buf"):
            self._test_inflater(expected_data, [b"".join(chunk) for chunk in _randomly_chunked(deflated_blocks)],
                                200 * 1024)

        with self.subTest("buf too small to inflate anything"):
            with bgzip.Inflater(1) as inflater:
                chunks = [b"".join(deflated_blocks)]
                inflate_info = inflater.inflate(chunks)
        ...
        with self.subTest("leading large chunk atomic"):
            with bgzip.Inflater(200 * 1024, atomic=True) as inflater:
                chunks = [b"".join(deflated_blocks[:-1]), deflated_blocks[-1]]
                inflate_info = inflater.inflate(chunks)
                self.assertEqual(inflate_info['remaining_chunks'], chunks)

        with self.subTest("trailing large chunk atomic"):
            with bgzip.Inflater(200 * 1024, atomic=True) as inflater:
                chunks = [deflated_blocks[0], b"".join(deflated_blocks[1:])]
                inflate_info = inflater.inflate(chunks)
    ...
    def test_inflate_streamed_chunk(self):
        ...
        with open("tests/fixtures/partial.vcf.gz", "rb") as raw:
            while True:
                ...
                if not input_buf:
                    break
                inflate_info = inflater.inflate([input_buf])
